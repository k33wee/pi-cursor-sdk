...
- Install dependencies: `npm install` (runs `prepare`, which compiles `src/` into `dist/` for scripts and programmatic consumers). Pi's manifest loads the shipped `src/index.ts` graph through its native TypeScript loader so Pi imports use host-owned peers even with local peer packages installed.
- Build after editing `src/`: `npm run build` remains required for validation, packaging, and compiled-script consumers. Restart Pi to pick up source changes. The cloud/steering/local-resume/provider-debug launchers rebuild automatically (even when run directly with `node scripts/...`), `smoke:live`/`smoke:visual`/`smoke:isolated` build via their npm scripts, and `smoke:platform*` builds inside its packed installs.
- Run tests: `npm test`
- The real npm package regression (`test/native-package-host.test.ts`) needs registry access or a warm npm cache for its package installs.
...
- Dependencies install with `npm install` (this triggers `prepare`, which compiles `src/` to `dist/` for scripts and programmatic consumers; Pi loads the shipped `src/index.ts` graph). After editing `src/`, run `npm run build` for validation and compiled consumers, then restart Pi.
